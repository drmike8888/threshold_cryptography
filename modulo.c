...
void mod_div(mpz_t a, mpz_t b, mpz_t c, mpz_t n)
{
  ...
  {
    printf("division by zero in mod_div!\n");
    mpz_clear(rslt);
    exit(-1);
  }
  mpz_mul(rslt, b, rslt);
...
/* random value with arbitrary modulus */

void mod_rand(mpz_t rand, mpz_t n)
{
...
/* return a random value in range
   0 to modulus - 1.  minit() and mseed() 
